...
Use the principal-enabled Harness `0.1.5-alpha.2` fork with `dsh-passwords 2.6.32`. Install all seventeen matching Mnemon packages and apply `deploy/server30-account.patch.yml`, replacing its absolute paths for other deployments. Install Native CLI `0.2.5` separately. Keep account storage outside user workspaces.
